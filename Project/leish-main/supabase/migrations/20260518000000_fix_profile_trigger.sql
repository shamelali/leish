-- Fix profile trigger to properly store role and log errors
-- Also ensures role is visible in Supabase Auth user metadata
      ...
      execute $sql$
        ...
        on conflict (id) do update
          set email = excluded.email,
              ...
              updated_at = now()
      $sql$
      using new.id, new.email, coalesce(new.raw_user_meta_data ->> 'full_name', split_part(new.email, '@', 1)), mapped_role_text;
    else
      execute $sql$
        insert into public.profiles (id, email, full_name, role)
        values ($1, $2, $3, $4::public.profile_role)
      ...
      execute $sql$
        ...
        on conflict (id) do update
          set full_name = excluded.full_name,
              ...
              updated_at = now()
      $sql$
      using new.id, coalesce(new.raw_user_meta_data ->> 'full_name', split_part(new.email, '@', 1)), mapped_role_text;
    else
      execute $sql$
        insert into public.profiles (id, full_name, role)
        values ($1, $2, $3)
      ...
      execute $sql$
        ...
        on conflict (id) do update
          set full_name = excluded.full_name,
              updated_at = now()
      $sql$
      using new.id, coalesce(new.raw_user_meta_data ->> 'full_name', split_part(new.email, '@', 1));
    else
      execute $sql$
        insert into public.profiles (id, full_name)
        values ($1, $2)

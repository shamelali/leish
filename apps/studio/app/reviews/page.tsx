...
import { getStudioDashboardData } from "@/lib/dashboard"

export const metadata: Metadata = {
  title: "Studio Reviews | Leish!",
...
const STATUS_LABEL: Record<string, string> = {
  published: "Published", hidden: "Hidden", escalated: "Escalated", pending: "Pending",
}
...
export default async function StudioReviewsPage() {
  ...
  const data = prov?.id ? await getStudioDashboardData(prov.id) : { reviews: [] }

  const total = data.reviews.length
  const avgRating = total > 0 ? (data.reviews.reduce((sum, r) => sum + r.rating, 0) / total).toFixed(1) : "—"
    ...
    <DashboardShell title="Reviews" subtitle="Track feedback quality and respond to customers." nav={nav}>
      ...
      <Panel title={`Review Inbox (${total})`}>
        <div className="space-y-3">
            ...
            <div key={r.id} className="border border-border bg-background p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex flex-col items-end gap-1">
                  <span className="font-mono text-[10px] uppercase text-muted-foreground">{STATUS_LABEL[r.status] ?? r.status}</span>
          ...
          {total === 0 && <p className="py-6 text-center text-sm text-muted-foreground">No reviews yet.</p>}
        </div>
      </Panel>
    </DashboardShell>
  )

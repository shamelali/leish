...
export default async function ProDashboardPage() {
  ...
  const data = providerId ? await getProDashboardData() : {
    ...
    stats: [], upcomingBookings: [], reviews: [], payouts: [],
  }
  ...
  const nav = [
    ...
    { href: "/charges", label: "Charges & Fees" },
  ]

  return (
    <DashboardShell title="Artist Dashboard" subtitle="Manage your artist profile, services, bookings, reviews, and payouts." nav={nav}>
      ...
      {isSuspended && (
        <div className="mb-6 rounded-lg border border-destructive bg-destructive/10 p-4">
          <p className="text-sm font-medium text-destructive">Your account is suspended. Please contact support for more information.</p>
        </div>
      )}
            ...
            <div className="space-y-3">
              {data.upcomingBookings.map((b, idx: number) => (
                <div key={`${b.date}-${b.slot}-${idx}`} className="flex items-center justify-between border border-border bg-background p-3">
                  <div>
                    <p className="font-serif text-base text-foreground">{b.date} &bull; {b.slot}</p>
                    <p className="text-xs text-muted-foreground">{b.client} &bull; {b.type}</p>
                  </div>
            ...
            <div className="space-y-2">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {data.provider.specialties.map((s) => (
                  <span key={s} className="border border-border px-2 py-1 font-mono text-[10px] uppercase text-muted-foreground">{s}</span>
                ))}
              </div>
            </div>
          </Panel>

              ...
              <div className="flex items-center gap-2">
                {b.status === "pending" && (
                  <form action={proConfirmBooking.bind(null, b.id)}>
                    <button type="submit" className="border border-border px-3 py-2 text-xs text-foreground hover:border-emerald-500 hover:text-emerald-600">Confirm</button>
                  </form>
                )}
                {["pending", "confirmed"].includes(b.status) && (
                  <form action={proCancelBooking.bind(null, b.id)}>
                    <button type="submit" className="border border-border px-3 py-2 text-xs text-foreground hover:border-red-500 hover:text-red-600">Cancel</button>
                  </form>
                )}
              </div>
            </div>

...
export async function GET(req: Request) {
  ...
  let rows
  if (dateKey) {
    const rawRows = await sql<{ id: string; starts_at: string; ends_at: string; is_booked: boolean }[]>`
      select * from public.availability_slots
      where provider_id = ${providerId} and starts_at::date = ${dateKey}::date
      order by starts_at
    `
    const cutoff = Date.now() + TWENTY_FOUR_HOURS_MS
    rows = rawRows.map((row) => ({
      id: row.id, slot: formatSlotLabel(row.starts_at),
      startsAt: row.starts_at, endsAt: row.ends_at,
      available: !row.is_booked && new Date(row.starts_at).getTime() >= cutoff,
    }))
  } else {
    rows = await sql`select * from public.availability_slots where provider_id = ${providerId} order by starts_at`
  }
...
function parseAvailabilityPayload(raw: unknown): { providerId: string; startsAt: string; endsAt: string } | null {
  ...
  return { providerId, startsAt, endsAt }
}
...
export async function POST(req: Request) {
  ...
  let parsed
  try { parsed = parseAvailabilityPayload(await req.json()) }
  ...
  const { data: prov } = await supabase.from("providers").select("owner_id").eq("id", parsed.providerId).maybeSingle()
...
export async function DELETE(req: Request) {
  ...
  const sql = getSql()
  const rows = await sql`select provider_id from public.availability_slots where id = ${slotId}`
  if (rows.length === 0) return NextResponse.json({ error: "Not found" }, { status: 404 })

  const { data: prov } = await supabase.from("providers").select("owner_id").eq("id", rows[0].provider_id).maybeSingle()
  if (prov?.owner_id !== user.id) return NextResponse.json({ error: "Not allowed" }, { status: 403 })

  await sql`delete from public.availability_slots where id = ${slotId}`

...
export default async function ArtistDashboardPage() {
  ...
  const nav = [
    { href: "/artist/dashboard", label: "Overview", active: true },
    ...
    <DashboardShell title="Artist Dashboard" subtitle="Manage your artist profile, services, bookings, reviews, and payouts." nav={nav}>
      ...
      {isSuspended && (
        <div className="mb-6 rounded-lg border border-destructive bg-destructive/10 p-4">
          <p className="text-sm font-medium text-destructive">Your account is suspended. Please contact support.</p>
        </div>
      )}
      ...
      {!isPro && !isSuspended && (
        <div className="mb-6 flex items-center justify-between rounded-lg border border-accent bg-accent/5 p-4">
          ...
          <Button asChild size="sm"><Link href="/pricing">Upgrade</Link></Button>
        </div>
      )}
